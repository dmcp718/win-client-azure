...
main() {
    ...
    if [ "$INSTALL_TC_BENCHMARK" = "1" ]; then
        ...
        # Encode tfbench.py and pyproject.toml as base64 to push via run-command
        # (Bitbucket zip download fails for private repos)
        TFBENCH_B64=$(base64 < "$(dirname "${BASH_SOURCE[0]}")/../tc-benchmark-files/tfbench.py" 2>/dev/null || base64 < "$(find / -maxdepth 5 -path '*/tc-benchmark/tfbench.py' -type f 2>/dev/null | head -1)" 2>/dev/null || echo "")
        PYPROJECT_B64=$(base64 < "$(dirname "${BASH_SOURCE[0]}")/../tc-benchmark-files/pyproject.toml" 2>/dev/null || base64 < "$(find / -maxdepth 5 -path '*/tc-benchmark/pyproject.toml' -type f 2>/dev/null | head -1)" 2>/dev/null || echo "")

        run_azure_cmd "Downloading TC Benchmark and tframetest" \
            "\$destPath = 'C:\\Users\\Public\\Desktop\\tc-benchmark'; if (Test-Path \$destPath) { Remove-Item -Path \$destPath -Recurse -Force }; New-Item -ItemType Directory -Path \$destPath -Force | Out-Null; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Write-Host 'Downloading tframetest from GitHub...'; New-Item -ItemType Directory -Path 'C:\\Temp' -Force | Out-Null; \$tframetestZip = 'C:\\Temp\\tframetest-win64.zip'; Invoke-WebRequest -Uri 'https://github.com/tuxera/tframetest/releases/download/3025.12.0/tframetest-win-x86_64-w64-mingw32-3025.12.0.zip' -OutFile \$tframetestZip -TimeoutSec 300 -UseBasicParsing; Expand-Archive -Path \$tframetestZip -DestinationPath 'C:\\Temp\\tframetest-extract' -Force; Copy-Item -Path 'C:\\Temp\\tframetest-extract\\tframetest-win-x86_64-w64-mingw32-3025.12.0\\tframetest.exe' -Destination \$destPath; Write-Host 'tframetest.exe installed'; Remove-Item -Path \$tframetestZip, 'C:\\Temp\\tframetest-extract' -Recurse -Force -ErrorAction SilentlyContinue; Write-Host 'Downloading tfbench.py from Bitbucket...'; \$tfbenchUrl = 'https://bitbucket.org/lucidlink/tc-benchmark/raw/master/tfbench.py'; \$pyprojectUrl = 'https://bitbucket.org/lucidlink/tc-benchmark/raw/master/pyproject.toml'; try { Invoke-WebRequest -Uri \$tfbenchUrl -OutFile (Join-Path \$destPath 'tfbench.py') -UseBasicParsing -TimeoutSec 60; Invoke-WebRequest -Uri \$pyprojectUrl -OutFile (Join-Path \$destPath 'pyproject.toml') -UseBasicParsing -TimeoutSec 60; \$tfSize = (Get-Item (Join-Path \$destPath 'tfbench.py')).Length; if (\$tfSize -lt 1000) { throw 'Downloaded file too small' }; Write-Host 'Downloaded from Bitbucket' } catch { Write-Host \"Bitbucket download failed (\$_), using embedded files...\"; \$tfB64 = '${TFBENCH_B64}'; \$pyB64 = '${PYPROJECT_B64}'; if (\$tfB64) { [IO.File]::WriteAllBytes((Join-Path \$destPath 'tfbench.py'), [Convert]::FromBase64String(\$tfB64)); [IO.File]::WriteAllBytes((Join-Path \$destPath 'pyproject.toml'), [Convert]::FromBase64String(\$pyB64)); Write-Host 'Installed from embedded files' } else { Write-Host 'WARNING: No embedded files available - tfbench.py not installed' } }; Write-Host \"TC Benchmark installed to \$destPath\"; Get-ChildItem \$destPath | ForEach-Object { Write-Host \"  \$(\$_.Name) (\$([math]::Round(\$_.Length/1KB, 1)) KB)\" }"
    else
